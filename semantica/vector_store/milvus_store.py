...
class MilvusStore:
    ...
    def _pymilvus_client(self):
        """Lazily build a pymilvus MilvusClient from this store's effective uri.

        Honors the constructor's uri/token (Milvus Lite file, remote server or
        Zilliz Cloud); falls back to http://host:port when no uri was given.
        ...
        if getattr(self, "_tri_client", None) is None:
            ...
            uri = self.uri or f"http://{self.host}:{self.port}"
            kwargs = {"token": self.token} if self.token else {}
            self._tri_client = _PyMilvusClient(uri=uri, **kwargs)
